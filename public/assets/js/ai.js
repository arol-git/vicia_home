...
document.addEventListener('DOMContentLoaded', () => {
    const rawConfig = JSON.parse(document.getElementById('ai-config')?.textContent || '{}');
    const config = {
        sendUrl: rawConfig.sendUrl || '/ai/message',
        resetUrl: rawConfig.resetUrl || '/ai/reset',
    };
    ...
    function sendMessage(text) {
        ...
        ViciaAjax.post(config.sendUrl, { message: text })
            ...
            .catch((err) => {
                ...
                const rawInfo = err.raw ? '\nRéponse serveur : ' + String(err.raw).slice(0, 240) : '';
                setStatus(null, '');
                appendMessage('assistant', '⚠️ ' + (err.message || "Une erreur est survenue.") + rawInfo);
            });
    }
    ...
    if (SpeechRecognition && secureContext) {
        ...
        recognizer.addEventListener('error', (event) => {
            ...
            const message = event.error === 'not-allowed' || event.error === 'service-not-allowed'
                ? 'Reconnaissance vocale impossible : autorisez le microphone et utilisez HTTPS ou localhost.'
                : `Reconnaissance vocale indisponible${reason}`;
            setStatus(null, message);
        });

        recognizer.addEventListener('nomatch', () => {
            setStatus(null, 'Je n’ai pas reconnu votre voix. Réessayez en parlant plus clairement.');
        });
        ...
        micBtn.title = SpeechRecognition
            ? 'Reconnaissance vocale activée uniquement sur HTTPS ou localhost.'
            : 'Reconnaissance vocale indisponible sur ce navigateur.';
        const label = micBtn.querySelector('.ai-mic-btn__label');
        if (label) {
            label.textContent = 'Voix indisponible';
        }
    }
