...
CREATE TABLE `users` (
    ...
    `remember_token`    VARCHAR(100)        NULL COMMENT 'jeton "Se souvenir de moi" de l’interface Web',
    `api_refresh_token` VARCHAR(100)        NULL COMMENT 'jeton de rafraîchissement JWT de l’API REST — colonne distincte de remember_token pour ne jamais invalider l’une des deux sessions en rafraîchissant l’autre',
...
CREATE TABLE `houses` (
    ...
    `mode`                  ENUM('confort','nuit','absence','urgence') NOT NULL DEFAULT 'confort' COMMENT 'mode courant de la maison, piloté depuis le Web ou le bot Telegram',
...
-- ---------------------------------------------------------------------
-- Table : devices
-- Modules ESP32 physiquement appairés à une maison. C'est cet
-- appairage (identifiant matériel unique + certificat) qui garantit
-- qu'un équipement/capteur est piloté par LA carte réellement
-- installée, et non par n'importe quel appareil publiant sur le même
-- nom de topic MQTT par coïncidence ou erreur de configuration.
-- ---------------------------------------------------------------------
CREATE TABLE `devices` (
    `id`                      INT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
    `house_id`                INT UNSIGNED    NOT NULL,
    `chip_id`                 VARCHAR(50)     NOT NULL COMMENT 'identifiant matériel unique de l’ESP32 (MAC ou chip ID)',
    `label`                   VARCHAR(100)    NOT NULL COMMENT 'nom donné par l’installateur, ex. "ESP32 salon"',
    ...
    KEY `idx_devices_house` (`house_id`),
...
-- ---------------------------------------------------------------------
-- Table : equipments
-- `device_id` identifie la carte ESP32 physique qui héberge réellement
-- cet équipement (nullable : un équipement peut être créé avant
-- l'appairage matériel définitif, ou piloté par une carte partagée).
-- ---------------------------------------------------------------------
CREATE TABLE `equipments` (
    ...
    KEY `idx_equipments_device` (`device_id`),
    CONSTRAINT `fk_equipments_room` FOREIGN KEY (`room_id`) REFERENCES `rooms` (`id`) ON DELETE CASCADE,
    CONSTRAINT `fk_equipments_device` FOREIGN KEY (`device_id`) REFERENCES `devices` (`id`) ON DELETE SET NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
CREATE TABLE `sensors` (
    ...
    CONSTRAINT `fk_sensors_device` FOREIGN KEY (`device_id`) REFERENCES `devices` (`id`) ON DELETE SET NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
-- Cartes ESP32 appairées à chaque maison (voir table `devices`).
INSERT INTO `devices` (`id`, `house_id`, `chip_id`, `label`, `status`, `last_seen`) VALUES
(1, 1, 'ESP32-A1B2C3D4E5F6', 'ESP32 Salon/Cuisine', 'paired', NOW()),
(2, 1, 'ESP32-112233445566', 'ESP32 Garage/Jardin', 'paired', NOW()),
(3, 2, 'ESP32-AABBCCDDEEFF', 'ESP32 Salon Douala', 'paired', NOW());

-- Équipements — topics MQTT préfixés par le slug de la maison
-- (home/<slug>/...), garantissant l'isolation des messages entre
-- maisons sur le même broker Mosquitto partagé. Chaque équipement est
-- rattaché à la carte ESP32 qui l'héberge réellement (`device_id`).
INSERT INTO `equipments` (`room_id`, `device_id`, `name`, `type`, `icon`, `state`, `mqtt_topic`) VALUES
(1, 1, 'Éclairage salon', 'led', 'fa-lightbulb', 1, 'home/villa-yaounde/lighting/salon/led1'),
(2, 1, 'Éclairage cuisine', 'led', 'fa-lightbulb', 0, 'home/villa-yaounde/lighting/cuisine/led1'),
(3, 1, 'Ventilateur chambre 1', 'ventilateur', 'fa-fan', 0, 'home/villa-yaounde/climate/chambre1/fan1'),
(4, 2, 'Porte de garage', 'porte', 'fa-warehouse', 0, 'home/villa-yaounde/security/garage/door1'),
(1, 1, 'Sirène d’alarme', 'sirene', 'fa-bell', 0, 'home/villa-yaounde/security/salon/siren1'),
(1, 1, 'Caméra salon', 'camera', 'fa-video', 1, 'home/villa-yaounde/camera/salon/cam1'),
(7, 3, 'Éclairage salon', 'led', 'fa-lightbulb', 1, 'home/residence-douala/lighting/salon/led1'),
(9, 3, 'Pompe d’arrosage', 'pompe', 'fa-faucet', 0, 'home/residence-douala/garden/terrasse/pump1');

-- Capteurs — même logique de préfixage par maison et de rattachement
-- à la carte ESP32 physique.
INSERT INTO `sensors` (`room_id`, `device_id`, `name`, `type`, `unit`, `icon`, `mqtt_topic`, `alert_threshold`) VALUES
(1, 1, 'PIR Salon', 'pir', 'bool', 'fa-walking', 'home/villa-yaounde/security/salon/pir', NULL),
(3, 1, 'Température chambre 1', 'dht22_temp', '°C', 'fa-temperature-high', 'home/villa-yaounde/climate/chambre1/temp', 35.00),
(3, 1, 'Humidité chambre 1', 'dht22_hum', '%', 'fa-tint', 'home/villa-yaounde/climate/chambre1/hum', NULL),
(2, 1, 'Détecteur de gaz cuisine', 'mq2', 'ppm', 'fa-smog', 'home/villa-yaounde/safety/cuisine/mq2', 400.00),
(6, 2, 'Humidité du sol jardin', 'humidite_sol', '%', 'fa-seedling', 'home/villa-yaounde/garden/jardin/soil', NULL),
(7, 3, 'PIR Salon', 'pir', 'bool', 'fa-walking', 'home/residence-douala/security/salon/pir', NULL),
(8, 3, 'Température chambre', 'dht22_temp', '°C', 'fa-temperature-high', 'home/residence-douala/climate/chambre/temp', 33.00);
