 ...
 * (administrateur de plateforme ou utilisateur client). Voir
 * App\Models\House pour les rôles propres à chaque maison.
 */
class User extends Model
{
    ...
    /**
     * Jeton de rafraîchissement de l'API REST (JWT) — volontairement
     * distinct de remember_token (session "Se souvenir de moi" de
     * l'interface Web) pour que rafraîchir l'un n'invalide jamais
     * l'autre : un même utilisateur peut être connecté simultanément
     * sur le site Web et sur un client API (bot Telegram, etc.).
     */
    public static function setApiRefreshToken(int $id, ?string $hashedToken): void
    {
        Database::query('UPDATE users SET api_refresh_token = :token WHERE id = :id', [
            'token' => $hashedToken,
            'id'    => $id,
        ]);
    }
