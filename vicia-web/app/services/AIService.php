...
class AIService
{
    public static function handle(int $userId, ?int $houseId, string $message): array
    {
        ...
        if (!$houseId) {
            return self::respond($conversation['id'], "Merci de sélectionner une maison avant de me parler d'équipements ou de capteurs — le reste, je peux déjà y répondre.", null, 'chitchat');
        }
