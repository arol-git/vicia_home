...
function api_house_apply_mode(int $houseId, string $mode): int
{
    $targets = [
        ...
        'night' => ['led' => 0, 'relais' => 0, 'ventilateur' => 0, 'pompe' => 0, 'porte' => 1, 'fenetre' => 1, 'sirene' => 0, 'camera' => 1],
        'away' => ['led' => 0, 'relais' => 0, 'ventilateur' => 0, 'pompe' => 0, 'porte' => 1, 'fenetre' => 1, 'sirene' => 0, 'camera' => 1],
        'emergency' => ['led' => 1, 'relais' => 1, 'ventilateur' => 1, 'pompe' => 0, 'porte' => 1, 'fenetre' => 1, 'sirene' => 1, 'camera' => 1],
    ];

    $sensorTargets = [
        'comfort' => ['pir' => 1, 'mq2' => 1, 'mq135' => 1, 'rfid' => 1, 'ldr' => 1, 'dht22_temp' => 1, 'dht22_hum' => 1, 'humidite_sol' => 1, 'energy_power' => 1, 'energy_kwh' => 1, 'energy_consumption' => 1],
        'night' => ['pir' => 1, 'mq2' => 1, 'mq135' => 1, 'rfid' => 1, 'ldr' => 1, 'dht22_temp' => 1, 'dht22_hum' => 1],
        'away' => ['pir' => 1, 'mq2' => 1, 'mq135' => 1, 'rfid' => 1, 'ldr' => 1, 'dht22_temp' => 1, 'dht22_hum' => 1],
        'emergency' => ['pir' => 1, 'mq2' => 1, 'mq135' => 1, 'rfid' => 1, 'ldr' => 1, 'dht22_temp' => 1, 'dht22_hum' => 1],
    ];
    ...
    foreach (\App\Models\Sensor::activeForHouse($houseId) as $sensor) {
        $type = $sensor['type'];
        if (!array_key_exists($type, $sensorTargets[$mode])) {
            continue;
        }

        $state = (int) $sensorTargets[$mode][$type];
        if ((int) $sensor['is_active'] === $state) {
            continue;
        }
