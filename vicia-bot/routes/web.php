...
use Bot\Controllers\SettingsController;

$router->command('/menu', [MenuController::class, 'index']);
$router->command('/aide', [HelpController::class, 'index']);
$router->command('/parametres', [SettingsController::class, 'index']);
$router->callback('menu:([a-z]+)', [MenuController::class, 'open']);

// --- Maison / Équipements / Capteurs / Caméras / Alarme / Réseau /
// Automatisation / Énergie (Modules 8-13) ---------------------------------
use Bot\Controllers\AlarmController;
use Bot\Controllers\AutomationController;
use Bot\Controllers\CameraController;
use Bot\Controllers\ConsumptionController;
use Bot\Controllers\EquipmentController;
use Bot\Controllers\NetworkController;
use Bot\Controllers\SensorController;

$router->callback('eq:toggle:(light|door):(\d+)', [EquipmentController::class, 'toggle']);
$router->callback('mode:set:(confort|nuit|absence|urgence)', [AlarmController::class, 'setMode']);
$router->callback('net:whitelist:(\d+)', [NetworkController::class, 'whitelist']);
$router->callback('net:blacklist:(\d+)', [NetworkController::class, 'blacklist']);
$router->callback('auto:toggle:(\d+)', [AutomationController::class, 'toggle']);

// --- Historique & rapports PDF (Module 14) --------------------------------
use Bot\Controllers\HistoryController;
use Bot\Controllers\ReportController;

$router->command('/historique', [HistoryController::class, 'index']);
$router->command('/rapport', [ReportController::class, 'generate']);

// --- Notifications poussées (Module 15) -----------------------------------
use Bot\Controllers\AlertWebhookController;

$router->callback('alert:read:(\d+)', [AlertWebhookController::class, 'markRead']);
