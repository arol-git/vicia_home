...
- `packagist.org` n'étant pas joignable depuis l'environnement de
  développement utilisé pour produire ce projet, `composer install`
  n'a jamais pu être exécuté réellement : tout le code a été testé
  contre des doublures fidèles à l'API réelle des bibliothèques
  (`irazasyed/telegram-bot-sdk`, `guzzlehttp/guzzle`,
  `vlucas/phpdotenv`, `monolog/monolog`). **Exécutez `composer install`
  et testez `/statut` avant toute mise en production.**
