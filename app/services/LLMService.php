...
PROMPT;
    ...
    public static function generateReply(string $userMessage, array $factualContext, array $recentMessages = []): string
    {
        $apiKey = self::apiKey();
        if (!$apiKey) {
            app_log('[AI] Clé API absente : configure AI_LLM_API_KEY.');
            return self::localReply($userMessage, $factualContext, $recentMessages, 'API missing');
        }

        $aiConfig = config('ai') ?: [];
        $provider = strtolower($aiConfig['provider'] ?? 'openai');
        ...
        try {
            ...
            app_log('[AI] Erreur LLM : ' . $e->getMessage());
            \App\Core\Session::flash('_ai_llm_error', $e->getMessage()); // à des fins de diagnostic ponctuel seulement
            return self::localReply($userMessage, $factualContext, $recentMessages, 'service unavailable');
        }
    }
    ...
    private static function callGemini(string $userMessage, array $context, array $recentMessages, string $apiKey): string
    {
        ...
        $url = self::baseUrl('gemini') . '?key=' . urlencode($apiKey);
        ...
        return self::fallbackReply($context);
    }
    ...
    private static function post(string $url, array $payload, array $headers): array
    {
        ...
        if ($status >= 400) {
            $message = $decoded['error']['message'] ?? $decoded['error'] ?? 'Erreur HTTP ' . $status;
            throw new \RuntimeException('Réponse LLM refusée : ' . (is_string($message) ? $message : json_encode($message, JSON_UNESCAPED_UNICODE)));
        }
     ...
     */
    private static function localReply(string $userMessage, array $context, array $recentMessages, string $reason): string
    {
        $shortContext = self::summarizeContext($context);
        $userMessage = trim($userMessage);

        if ($reason === 'API missing') {
            if (empty($shortContext)) {
                return "Le moteur IA externe n'est pas configuré. Je peux toujours piloter la maison et répondre aux états connus.";
            }
            return "Le moteur IA externe n'est pas configuré. Voici ce que je peux dire en local : " . $shortContext;
        }
    ...
    private static function summarizeContext(array $context): string
    {
        ...
        foreach ($context as $key => $value) {
            $valueText = self::describeValue($value);
            if ($valueText !== '') {
                $parts[] = match ($key) {
                    'sensors' => 'Capteurs : ' . $valueText,
                    'equipments' => 'Équipements : ' . $valueText,
                    'unread_critical_alerts' => "Alertes critiques non lues : $valueText",
                    'devices' => 'Appareils réseau : ' . $valueText,
                    'total_active_watts' => 'Puissance active estimée : ' . $valueText,
                    'estimated_daily_kwh' => 'Consommation journalière estimée : ' . $valueText,
                    default => ucfirst(str_replace('_', ' ', $key)) . ' : ' . $valueText,
                };
            }
        }
    ...
    {
        return self::localReply('', $context, [], 'service unavailable');
    }
    ...
    {
        $aiConfig = config('ai') ?: [];
        $key = $aiConfig['api_key'] ?? getenv('AI_LLM_API_KEY');
        return $key ?: null;
    }
